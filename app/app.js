const TRANSLATION_URL_GOOGLE =
  "https://translate.google.com/?sl=ja&tl=en&text=%s&op=translate";
...
const _loadRom = function (arrayBuffer, fileName) {
  ...
  currentPointers.forEach(function (pointer) {
    ...
    header.innerHTML =
      ...
      "</strong> <small>0x" +
      pointer.pointerOffset.toString(16).padStart(6, "0") +
      " &rarr; 0x" +
      pointer.pointer.toString(16).padStart(6, "0") +
      "</small><br/>";
    header.innerHTML +=
      '<div class="comment">' +
    ...
    commentContainer.addEventListener("click", function () {
      ...
      if (knownPointer) {
        if (newComment) knownPointer.comment = newComment;
        else delete knownPointer.comment;
      } else if (newComment) {
        KNOWN_POINTERS.push({
    ...
    textareaTranslated.value = pretranslatedText
      ? knownPointer.translation
      : "";
  ...
  GRAPHIC_REPLACEMENTS.filter(
    ...
    rowDiv.appendChild(document.createElement("div"));
    rowDiv.appendChild(document.createElement("div"));
    rowDiv.children[0].innerHTML =
      "<strong>Graphic replacement #" +
      i +
      "</strong> <small>0x" +
      graphicReplacement.offset.toString(16).padStart(6, "0") +
      "</small><br/>";
    rowDiv.children[0].innerHTML +=
      "<div>" + (graphicReplacement.comment || "-") + "</div>";
      ...
      imgTranslated.onload = function (evt) {
        ...
        if (image.width % 8 !== 0 || image.height % 8 !== 0) {
          ...
          divMessage.innerHTML =
            graphicReplacement.file +
            ".png width and height must be multiple of 8 (" +
            image.width +
            "x" +
            image.height +
            "px)";
        ...
        if (result.tileset.tiles.length > graphicReplacement.nTiles) {
          ...
          divMessage.innerHTML =
            graphicReplacement.file +
            ".png exceeds tile limit (&gt;" +
            graphicReplacement.nTiles +
            " tiles)";
          rowDiv.children[2].appendChild(divMessage);
        }

        const patchData = result.tileset.tiles
          .map((tile) => tile.export())
          .flat();
        PATCHES.push({
          offset: graphicReplacement.offset,
          name:
            "automatic generated graphics patch: " + graphicReplacement.file,
          data: patchData,
        });
  ...
  GRAPHIC_REPLACEMENTS.filter(
    ...
    rowDiv.appendChild(document.createElement("div"));
    rowDiv.children[0].innerHTML =
      "<strong>Map replacement #" +
      i +
      "</strong> <small>Tileset: 0x" +
      graphicReplacement.offsetTileset.toString(16).padStart(6, "0") +
      "</small> <small>Map: 0x" +
      graphicReplacement.offsetMap.toString(16).padStart(6, "0") +
      "</small><br/>";
    rowDiv.children[0].innerHTML +=
      "<div>" + (graphicReplacement.comment || "-") + "</div>";
    ...
    if (graphicReplacement.file) {
      ...
      imgTranslated.onload = function (evt) {
        const image = this;
        if (
          image.width !== graphicReplacement.width * 8 ||
          image.height !== (graphicReplacement.height + 1) * 8
        ) {
          const divMessage = document.createElement("div");
          divMessage.className = "text-danger";
          divMessage.innerHTML =
            graphicReplacement.file +
            ".png width and height must be " +
            graphicReplacement.width * 8 +
            "x" +
            (graphicReplacement.height + 1) * 8 +
            "px";
          rowDiv.children[2].appendChild(divMessage);
        }
        const canvas = document.createElement("canvas");
        ...
        const patchDataTileset = result.tileset.tiles
          .map((tile) => tile.export())
          .flat();
        PATCHES.push({
          offset: graphicReplacement.offsetTileset,
          name:
            "automatic generated graphics (tileset) patch: " +
            graphicReplacement.file,
        ...
        PATCHES.push({
          ...
          name:
            "automatic generated graphics (map) patch: " +
            graphicReplacement.file,
          data: patchDataMap,
        });
...
window.addEventListener("load", function (evt) {
  ...
  document
    ...
    .addEventListener("click", function (evt) {
      ...
      let textareaValue = "";
      currentPointers
        .filter((pointer) => pointer.modified > 1)
  ...
  document
    ...
    .addEventListener("click", function (evt) {
      // Crear input para seleccionar archivo JSON
  ...
  this.document.getElementById("progress-status").value =
    translationStatus.done;
  this.document.getElementById("span-status-done").innerHTML =
    translationStatus.done;
  this.document.getElementById("span-status-total").innerHTML =
    translationStatus.total;

  if (
    location.protocol === "file:" &&
    typeof GAME_INFO.autoloadFile === "string"
  ) {
    _showLoadingMessage("Loading ROM...");
