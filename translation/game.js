const GAME_INFO = {
  ...
  saveTexts: function (romFile, texts) {
    ...
    texts.forEach(function (pointerInfo) {
      if (freeOffset + pointerInfo.data.length >= currentSafeZone.end) {
        ...
        console.info(
          "jumping to safe zone " +
            (currentZoneIndex + 1) +
            " at pointer #" +
            pointerInfo.pointerIndex,
        );
    ...
    PATCHES.filter(
      (patchInfo) => patchInfo.data && Array.isArray(patchInfo.data),
    ).forEach(function (patchInfo, i) {
    ...
    while (text.length) {
      if (/^\[[0-9a-f]{2,4}\]/i.test(text)) {
        ...
        if (/^\[[a-z0-9]{3,5}\]/i.test(text)) {
          ...
          const specialChar = CHAR_TABLE.find(
            (char) => char.char === wordMatch[0],
          );
          if (specialChar) {
        ...
        const foundChar = CHAR_TABLE.find(
          (char) => char.char === text.charAt(0),
        );
        if (foundChar) {
  ...
  checkPointerDataContinuity: function () {
    const pointersSortedByOffset = currentPointers.map(function (t) {
      ...
      if (knownPointer) pointerData.comment = knownPointer.comment;
...
const CHAR_TABLE = [
  ...
  { id: 0x014d, char: "Á" }, //unused, safe to use for accents?
  { id: 0x014e, char: "É" }, //unused, safe to use for accents?
  { id: 0x014f, char: "Í" }, //unused, safe to use for accents?
  { id: 0x0150, char: "悪" },
  { id: 0x0151, char: "員" },
  { id: 0x0152, char: "回" },
  { id: 0x0153, char: "換" },
